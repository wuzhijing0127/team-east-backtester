# v4p_aggressor — v4j with aggressor-follow signal on ASH
"""
...
def ash_aggressor_bias(state, ts: dict) -> int:
    """
    Return a signed bias from recent ASH market-trade flow.
    +1 = recent buyer aggression (bias toward buying / widening ask)
    -1 = recent seller aggression (bias toward selling / widening bid)
     0 = neutral
    Uses a short EMA of net aggressor volume.
    """
    product = "ASH_COATED_OSMIUM"
    # Trades that occurred since last state
    ...
    net = 0
    for t in recent:
        # Aggressor inferred from price vs prev mid: above = buy-aggr, below = sell-aggr
        try:
            if t.price > prev_mid:
                net += t.quantity
...
def trade_ash(state: TradingState, ts: dict) -> List[Order]:
    ...
    if best_bid is not None and best_ask is not None:
        ...
        # v4p: overlay aggressor-follow bias
        agg_bias = ash_aggressor_bias(state, ts)
        if agg_bias != 0 and inv_bias == 0:
            inv_bias = agg_bias
    ...
    # v4p: track prev_mid for next-tick aggressor inference
    m = get_mid(od)
    if m is not None:
        ts["ash_prev_mid"] = m
