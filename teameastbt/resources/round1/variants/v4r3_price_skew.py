# v4r3 — v4r2 (graded skew) + directional PRICE SKEW
# alpha=0.15, noise_gate=2.0, conf_min=0.20, spread_gate=3
# size skew 0.20..1.00, price shift max = 1 tick at full confidence
"""
...
def trade_ash(state: TradingState, ts: dict) -> List[Order]:
    ...
    if best_bid is not None and best_ask is not None:
        ...
        # v4r3: graded signal + size skew + PRICE SKEW (new)
        ...
        # Confidence fraction in [0, 1] for overlay-active region
        t = 0.0
        if overlay_active:
            t = (confidence - 0.20) / (1.0 - 0.20)
            t = max(0.0, min(1.0, t))
        ...
        # --- Price skew (NEW): shift quote prices in signal direction ---
        # Conservative first cut: max 1 tick at full confidence.
        # Bullish: shift both +1 → tighter bid (more aggressive), looser ask (less exposed).
        # Bearish: mirror.
        price_shift_max_ticks = 1
        if overlay_active:
            shift = int(round(t * price_shift_max_ticks))
            if direction > 0:
                l1_bid += shift
                l1_ask += shift
            else:
                l1_bid -= shift
                l1_ask -= shift
            # Keep quotes non-crossing and bounded by the visible book
            l1_bid = max(best_bid, min(l1_bid, best_ask - 1))
            l1_ask = min(best_ask, max(l1_ask, best_bid + 1))
