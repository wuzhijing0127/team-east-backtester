# v4n_ladder_big — multi-tier improve at best+1/+2/+3 (sizes 14/10/8)
"""
...
ASH_PARAMS = {
    ...
    "narrow_spread_thr": 4,
    # Multi-tier improve ladder: list of (tick_improve, size) — posts at best±tick
    "ash_ladder": [(1, 14), (2, 10), (3, 8)],
}
...
def trade_ash(state: TradingState, ts: dict) -> List[Order]:
    ...
    if best_bid is not None and best_ask is not None:
        ...
        # --- Multi-tier improve ladder: post at best+1, best+2, best+3 ---
        mkt_spread = best_ask - best_bid
        for tick, tier_size in p["ash_ladder"]:
            if mkt_spread <= 2 * tick:
                continue  # not enough room to improve by `tick` on both sides
            bid_p = best_bid + tick
            ask_p = best_ask - tick
            if bid_p >= ask_p:
                continue
            buy_q = min(round(tier_size * buy_mult), remaining_buy)
            sell_q = min(round(tier_size * sell_mult), remaining_sell)
            # Inventory-bias: suppress the side that increases exposure when heavy
            if inv_bias < 0:
                buy_q = min(buy_q, round(tier_size * 0.3))
            elif inv_bias > 0:
                sell_q = min(sell_q, round(tier_size * 0.3))
            if buy_q > 0:
                orders.append(Order(product, bid_p, buy_q))
                remaining_buy -= buy_q
            if sell_q > 0:
                orders.append(Order(product, ask_p, -sell_q))
                remaining_sell -= sell_q
