# v4q2 — same skew as v4r2, plus extremely selective cross-spread branch
# cross fires only at conf>=0.90, persistence>=2 ticks, spread>=5, near-neutral pos
# cross_size: 1 unit normally, 2 at conf>=0.95
"""
...
def ash_signal(state, ts: dict):
    """
    (direction, confidence) from ASH aggressor flow.
    alpha=0.15, noise_gate=2.0, saturation at |ema|=15.
    """
    recent = getattr(state, "market_trades", {}).get("ASH_COATED_OSMIUM", []) or []
    ...
    ema = 0.15 * net + (1 - 0.15) * ema
    ts["ash_aggr_ema"] = ema
    mag = abs(ema)
    if mag < 2.0:
        return 0, 0.0
    direction = 1 if ema > 0 else -1
    confidence = min(1.0, mag / 15.0)
...
def trade_ash(state: TradingState, ts: dict) -> List[Order]:
    ...
    if best_bid is not None and best_ask is not None:
        ...
        # v4q2: graded signal, don't-trade zones, base+overlay separation
        ...
        # Continuous skew: 0.2 (at conf=0.2) → 1.0 (at conf=1.0)
        buy_skew = 1.0
        sell_skew = 1.0
        if overlay_active:
            # Linear map: [overlay_conf_min, 1.0] → [min_skew, max_skew]
            t = (confidence - 0.2) / (1.0 - 0.2)
            t = max(0.0, min(1.0, t))
            scale = 0.2 + t * (1.0 - 0.2)
            if direction > 0:
                buy_skew = 1.0 + scale
                sell_skew = max(0.0, 1.0 - scale)
            else:
                buy_skew = max(0.0, 1.0 - scale)
                sell_skew = 1.0 + scale

        # --- L1 base layer: quotes always on; sizes scaled by skew ---
        ...
        # --- Overlay cross: extremely selective ---
        if overlay_active and confidence >= 0.9:
            # Persistence: require 2 consecutive ticks same direction
            prev_dir = ts.get("ash_dir_streak_dir", 0)
            streak = ts.get("ash_dir_streak", 0)
            if prev_dir == direction:
                streak += 1
            else:
                streak = 1
            ts["ash_dir_streak"] = streak
            ts["ash_dir_streak_dir"] = direction

            # Gate: wider spread requirement + near-neutral inventory
            near_neutral = abs(pos) <= limit * 0.3
            if streak >= 2 and mkt_spread >= 5 and near_neutral:
                cross_qty = 1 if confidence < 0.95 else 2
            ...
            # Reset streak when overlay not active or confidence below cross threshold
            if ts.get("ash_dir_streak_dir") != direction:
                ts["ash_dir_streak"] = 0
                ts["ash_dir_streak_dir"] = direction
