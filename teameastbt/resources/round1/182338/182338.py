"""
Round 1 — PEPPER pure buy-and-hold, ASH unchanged from original.
"""
...
# ASH — identical to round1_str original
ASH_PARAMS = {
    "position_limit": 50,
    "anchor_fair": 10000,
    "micro_beta": 0.0,
...
# Drift = +0.001/tick = +1000/day. Position limit = 50.
# Theoretical max = 50 * 1000 = 50,000/day.
# Optimal strategy: get to max long ASAP, never sell.
# Any selling loses drift profit. Any complexity adds risk for <1% gain.
...
def trade_pepper(state: TradingState, ts: dict) -> List[Order]:
    ...
    # buy all visible asks until full
    if best_ask is not None:
        for ask_price in sorted(od.sell_orders.keys()):
            vol = abs(od.sell_orders[ask_price])
            qty = min(vol, limit - pos)
    ...
    # if not full, leave one aggressive passive bid
    remaining = limit - pos
    if remaining > 0:
