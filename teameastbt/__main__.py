...
from teameastbt.data import has_day_data
from teameastbt.file_reader import FileReader, FileSystemReader, PackageResourcesReader
from teameastbt.metrics import format_risk_metrics_block, risk_metrics_full_period
from teameastbt.models import BacktestResult, TradeMatchingMode
from teameastbt.open import open_visualizer
from teameastbt.runner import run_backtest


def parse_algorithm(algorithm: Path) -> Any:
    sys.path.append(str(algorithm.parent))

    from teameastbt import datamodel
...
def version_callback(value: bool) -> None:
    if value:
        print(f"teameastbt {metadata.version(__package__)}")
        sys.exit(0)
...
def cli(
    ...
    data: Annotated[Optional[Path], Option(help="Path to data directory. Must follow the round{N}/prices_round_{N}_day_{D}.csv structure.", show_default=False, exists=True, file_okay=False, dir_okay=True, resolve_path=True)] = None,
