"""
r2_ash_limit50_v1 — ASH limit raised 20→50 with layered anti-bleed formulas
=============================================================================

Hypothesis: at limit=50, ASH still bleeds via adverse selection unless
we change HOW we trade. Larger pos cap = larger MtM swing per mid move,
so defensive logic has to scale up too.

Anti-bleed stack (in order of application):
  1. Wider take edge (2 instead of 1) — rejects shallow +0/+1 edges that
     historically flipped adverse inside a few ticks
  2. Stronger inventory skew (k_inv=4.0) — reservation price pulled
     harder toward flat; quotes widen fast on loaded side
  3. Size-decay formula: posting size shrinks linearly as position
     loads, not just clamped at the wall. At |pos|=25 (50% loaded),
     we post half the base_size; at full, we post zero.
  4. Smaller base_size (5 vs 8) — compensates for 2.5× the cap.

PEPPER: drawdown_v1 engine (v16 core + tail-guard) — unchanged.
"""
...
# --- ASH (limit 50 with anti-bleed formulas) ---
ASH_LIMIT = 50
ASH_INIT_FAIR = 10000
ASH_EMA_ALPHA = 0.05
ASH_HALF_SPREAD = 1
...
def trade_ash(state, ts):
    ...
    # Take: wider edge (2) — reject shallow flips
    if ba is not None:
    ...
    # MM: stronger inventory skew + size-decay by inventory
    ...
    # Size-decay: fraction = 1 - |pos|/LIMIT  (linear fade as we load the book)
    load_frac = abs(pos) / ASH_LIMIT if ASH_LIMIT > 0 else 0.0
    decayed = max(0, int(round(ASH_BASE_SIZE * (1 - load_frac))))
    bid_size = min(decayed, ASH_LIMIT - pos)
    ask_size = min(decayed, ASH_LIMIT + pos)
    if bid_size > 0 and pos < ASH_LIMIT:
        orders.append(Order(prod, bid_px, bid_size))
    if ask_size > 0 and pos > -ASH_LIMIT:
        orders.append(Order(prod, ask_px, -ask_size))
    return orders
...
def trade_pepper(state, ts):
    ...
    if reduce_only:
        if pos > 0 and ba is not None:
            ...
            orders.append(Order(prod, ba + 1, -qty))
        return orders
