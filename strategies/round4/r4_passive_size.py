...
# ── PARAMS_BLOCK_START ──
VARIANT_ID = 'passive_size'
VARIANT_LABEL = 'round() + passive_size 60/40 (maker quotes bigger; takes still 20/15)'
...
def trade_flow_mm(state: TradingState, ts: dict, product: str, p: dict) -> List[Order]:
    ...
    # Passive maker quote size — decoupled from base_size (which caps takes).
    # Bigger passive quotes give Marks more room to fill from us per tick;
    # smaller take size keeps aggressive crosses conservative.
    passive_size = p.get("passive_size", base_size)
    room_buy = max(0, soft_cap - pos)
    room_sell = max(0, soft_cap + pos)
    b_size = min(passive_size, room_buy)
    a_size = min(passive_size, room_sell)
...
# nonce: 2026-04-26T17:35:07.465311 669390723