"""
Round 4 Assets-Only Strategy — slim momentum engine.
...
Active mechanisms (only):
  ...
  - ema_alpha smoothing on composed fair   # stability anchor (dominant alpha)
  - k_inv symmetric inventory shade        # negative-feedback inventory control
  - catastrophic insurance stop-loss       # circuit breaker on stuck inventory

Removed (all dead/inert in winning variant):
  - mean-reversion, asymm_skew, vol_widen_coef, deep_imb_coef
  - micro_trend_coef, imb_window, long_ema_alpha, ema_blend
  - target_pos layer, conviction-relief, asymmetric quoting
  - one-sided gate, dynamic take size, chop regime, diagnostics
"""
...
# ── PARAMS_BLOCK_START ──
VARIANT_ID = 'slim_winner'
VARIANT_LABEL = 'Slim engine — 17 params per asset, all dead branches removed'
...
def trade_flow_mm(state: TradingState, ts: dict, product: str, p: dict) -> List[Order]:
    ...
    # ── Composed fair: mid + momentum + book-flow tilts ──
    fair = (mid
    ...
    # ── Optional EMA smoothing on fair ──
    if p["ema_alpha"] > 0:
        ema_key = f"{product}_fair_ema"
        prev = ts.get(ema_key, fair)
        fair = p["ema_alpha"] * fair + (1 - p["ema_alpha"]) * prev
        ts[ema_key] = fair
...
# nonce: 2026-04-26T21:25:49.777321 3273244
