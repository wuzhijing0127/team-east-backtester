...
Three engines, each transplanted whole from its source-of-truth:
  - VELVETFRUIT_EXTRACT — flow-MM with sweep-frozen alpha (round4_vfe_only.py
    + sweep_vfe state). Uses the full-schema trade_flow_mm that HG also uses;
    legacy signal flags are zeroed out so behaviour matches the slim
    sweep-tested logic.
  ...
  - 10 VEV options      — entire options engine from options_strat.py
    ...
    Rescaled to the 1M platform timescale (was 100K in source).

Time scale: 1_000_000 (per user — 'final run will be possibly 998000').
  - HG / VFE flow-MM closeout: ts >= 998_000
  - Options taper-unwind:      ts >= 990_000
  - Options hard unwind:       ts >= 998_000
"""
...
UNDERLYING_UNWIND_START  = 998_000   # HG / VFE flow-MM closeout
HARD_UNWIND              = 998_000   # options-engine hard liquidation
UNWIND_START_OPTIONS     = 990_000   # options-engine taper start
...
P = {
    ...
    "unwind_start": UNWIND_START_OPTIONS,   # 990_000 — rescaled from 99_000
}
