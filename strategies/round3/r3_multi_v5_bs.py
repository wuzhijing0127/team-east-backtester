...
# v5: Black-Scholes engine
import math
TICKS_PER_DAY = 1_000_000
DEFAULT_IV = 0.24  # initial sigma (matches empirical implied vol at TTE=6d)
...
def implied_vol_call(target_price: float, S: float, K: float, T: float,
                     r: float = 0.0, tol: float = 1e-3, max_iter: int = 25):
    """Bisection-search implied vol from a market price. Returns None if no solution."""
    intrinsic = max(0.0, S - K)
    if target_price <= intrinsic + 1e-6 or T <= 0:
        return None
    lo, hi = 0.01, 3.0
...
def get_tte_years(timestamp: int) -> float:
    """Continuous TTE in years. Round 3 starts at 5 days, decays linearly to ~4 days
    by end of session (1M ticks). Floor at 0.5 days to avoid degenerate BS at expiry."""
    days_remaining = TTE_AT_ROUND_START - (timestamp / TICKS_PER_DAY)
    return max(0.5 / 365.0, days_remaining / 365.0)


def update_implied_vol(state, ts: dict, vf_mid):
    """Update rolling implied vol from VEV_5200 mid (closest to ATM). Smoothed via EMA."""
    if vf_mid is None:
        return ts.get("bs_iv", DEFAULT_IV)
    od = state.order_depths.get("VEV_5200")
    if od is None:
        return ts.get("bs_iv", DEFAULT_IV)
    bid, ask = (max(od.buy_orders) if od.buy_orders else None,
                min(od.sell_orders) if od.sell_orders else None)
    if bid is None or ask is None:
        return ts.get("bs_iv", DEFAULT_IV)
    mid_5200 = (bid + ask) / 2.0
    T = get_tte_years(state.timestamp)
    iv_now = implied_vol_call(mid_5200, vf_mid, 5200, T)
    if iv_now is None or not (0.05 < iv_now < 1.5):
        return ts.get("bs_iv", DEFAULT_IV)
    prev = ts.get("bs_iv", iv_now)
    new_iv = 0.9 * prev + 0.1 * iv_now  # EMA-smooth
    ts["bs_iv"] = new_iv
    return new_iv


def get_total_vev_delta(state, ts: dict = None) -> float:
    """Sum pos*delta across every VEV strike using BS deltas if state context available,
    falling back to static VEV_DELTA dict otherwise."""
    total = 0.0
    if ts is not None:
        vf_mid = ts.get("vf_mid")
        if vf_mid is not None:
            T = get_tte_years(state.timestamp)
            iv = ts.get("bs_iv", DEFAULT_IV)
...
def trade_vev_near(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # v5_BS: blend BS theoretical fair with EMA-of-mid (70% BS / 30% EMA) for stability
    vf_mid = ts.get("vf_mid")
    iv = update_implied_vol(state, ts, vf_mid)
    T_years = get_tte_years(timestamp)
    if vf_mid is not None:
        bs_fair = bs_call_price(vf_mid, strike, T_years, iv)
        fair = 0.7 * bs_fair + 0.3 * ema_fair
    else:
        fair = ema_fair
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            if abs(total_delta) >= 250:
                vf_pos = state.position.get(vf_sym, 0)
                target_vf = -round(0.75 * total_delta)
