"""
Round 3 Strategy — r3_multi_v11_a (strike-bucketed blend weights)
==================================================================
Branch A from v10_blend: vary the BS/EMA blend weight by strike bucket
instead of using a uniform 0.3/0.7. Wings get more BS structure (their mids
are noisy with low absolute prices), middle stays heavily market-anchored.

  K in (4000, 4500):       w_bs = 0.20, w_ema = 0.80   (deep ITM — let market lead)
  K in (5000, 5100, 5200): w_bs = 0.25, w_ema = 0.75   (cluster — market dominant)
  K in (5300, 5400, 5500): w_bs = 0.40, w_ema = 0.60   (wings — BS stabilizes thin mids)

Other v10_blend pieces preserved unchanged: per-strike bisection IV for delta,
inventory shading, hedge trigger 150 / weight 0.95 / cap ±30.
...
def trade_vev_bs(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # Branch A: strike-bucketed blend weights.
    if strike in (4000, 4500):
        ...
        w_bs = 0.40
    fair = w_bs * bs_fair + (1.0 - w_bs) * ema_fair
