...
def trade_vev_near(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    resid_thresh = max(0.5, vega_k * SMILE_RESID_SD)
...
def trade_vev_far_otm(state: TradingState, ts: dict, strike: int) -> List[Order]:
    """v6 PHASE 3.1: Far-OTM (6000/6500) is pegged at 0.5. Harvest the floor by
    quoting a tight bid/ask around it. No vol risk (delta ≈ 0), no theta risk
    (already worth zero). Just grab the spread when someone trades through us.
    """
    product = f"VEV_{strike}"
    od = state.order_depths.get(product)
    if od is None:
    ...
    # Floor-pinned market: best_bid is normally 0, best_ask normally 1.
    # Quote 0/1 — passive on both sides — collect the 1-tick spread on flow.
    soft_cap = 60     # don't accumulate more than 60 lots either way
    base_size = 10
    if pos < soft_cap:
        # buy at 0 (floor)
        bid_px = 0
        ...
        if bid_px >= 0:
            buy_qty = min(base_size, soft_cap - pos)
            if buy_qty > 0:
