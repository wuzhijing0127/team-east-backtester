"""
Round 3 Strategy — r3_multi_v11_c (stronger inventory shading)
================================================================
Branch C from v10_blend: ramp the inventory shade earlier and steeper.

  v10_blend: shade kicks in at |pos|>200, max 1 tick
  v11_c:     shade kicks in at |pos|>100, max 2 ticks
             shade(p) = min(2.0, max(0, |p|-100) / 100)

  Effect at sample positions:
    |pos|=100  → 0 ticks   (was 0)
    |pos|=200  → 1 tick    (was 0)
    |pos|=250  → 1.5 ticks (was 0.5)
    |pos|=300  → 2 ticks   (was 1)

Bot quotes more conservatively earlier when building inventory, helping it
unwind passively before reaching the limit.

Other v10_blend pieces preserved: market-anchored 0.3/0.7 blend, bisection
IV for delta, hedge trigger 150 / weight 0.95 / cap ±30.

(v10_blend docstring follows)
...
def trade_vev_bs(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    if abs(pos) > 100:
        # v11_c: ramp earlier (>100) and steeper (max 2 ticks at limit).
        shade = min(2.0, (abs(pos) - 100) / 100.0)
        if pos > 0:
            fair -= shade
        else:
            fair += shade
