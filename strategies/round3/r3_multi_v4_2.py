"""
Round 3 Strategy — r3_multi_v4.2
================================
...
def trade_vev_near(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # Emergency cross at extreme single-strike position — CLUSTER ONLY.
    # 5300/5400/5500 are small-dollar options where spread cost would dominate.
    if in_cluster and pos > 240 and best_bid is not None:
        cross_qty = 22 if pos > 280 else 12
        cross_qty = min(cross_qty, pos, limit + pos)
        if cross_qty > 0:
            orders.append(Order(product, best_bid, -cross_qty))
    elif in_cluster and pos < -240 and best_ask is not None:
        cross_qty = 22 if pos < -280 else 12
        cross_qty = min(cross_qty, -pos, limit - pos)
        if cross_qty > 0:
