"""
Round 3 Strategy — r3_multi_v10_recal (data-fit smile)
=======================================================
Same scaffold as v8 but with the SMILE COEFFICIENTS REFIT from historical
market mids (1,800 points across days 0/1/2, K=5000..5500, TTE 8/7/6 days):

  sigma(m) = 0.5174 * m² + 0.0030 * m + 0.01208
            (old: 2.308·m² + 0.0526·m + 0.02047)

This is roughly 60% the magnitude of the original analyst's parabola. The
old SMILE_C = 0.02047 was producing σ values that gave BS prices systematically
above market — exactly the bias that caused v9 to over-buy at the ask after
vol_offset saturated.

Plus the two safe v9 changes:
  Q2_KEPT. Inventory shading: |pos|>200 → shade up to 1 tick against pos.
  Q3_KEPT. Hedge trigger 150 (was 250).
...
# v10_recal: parabola refit on 1,800 historical points (days 0/1/2, K=5000..5500).
SMILE_A = 0.5174
SMILE_B = 0.0030
SMILE_C = 0.01208
...
def smile_iv(K: float, S: float, T_days: float) -> float:
    """Fitted parabolic smile in normalized log-moneyness.
    Returns model sigma (>=0.001) for use in BS pricing."""
    ...
    if ts is not None:
        ...
        if vf_mid is not None:
            ...
            for strike in VEV_DELTA:
                ...
                if pos == 0:
                    continue
                sigma_k = smile_iv(strike, vf_mid, T)
...
def trade_vev_bs(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    if vf_mid is None or strike not in SMILE_STRIKES:
        # Defensive fallback: if VF not yet observed, hold.
        return []
    sigma_k = smile_iv(strike, vf_mid, T_days)
    bs_fair = bs_call_price(vf_mid, strike, T_days, sigma_k)
    fair = bs_fair

    # v10_recal Q2_KEPT: inventory shading.
    if abs(pos) > 200:
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            if abs(total_delta) >= 150:  # v10_recal Q3_KEPT: 250 -> 150
