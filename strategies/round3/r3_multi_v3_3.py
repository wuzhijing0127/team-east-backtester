"""
Round 3 Strategy — r3_multi_v3.3
================================
v3.3 deltas vs v3.2 — portfolio-level option risk control. v3.2 backtest showed
VEV_5000/5100/5200 losing in lockstep around t≈59400 because the bot was long
all three; one underlying weakening hits the whole basket. Fixes:
  ...
  P3. VF DELTA HEDGE: in run(), after VEV orders, append a 0.35x partial VF
      hedge order capped at ±20/tick so a long-call basket gets offset by short VF.
  P4. CLUSTER EDGE WIDEN: in trade_vev_near, if strike in {5000,5100,5200}
      and cluster_pos > 300, take_edge += 2 and fair -= 1.0 (don't keep adding
      to a correlated long stack).
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # v3.3 P3: partial VF delta hedge — offset 35% of total VEV delta with VF
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            target_vf = -round(0.35 * total_delta)
            hedge_qty = max(-20, min(20, target_vf - vf_pos))
            # Respect VF limit on the side we're hedging into
            if hedge_qty > 0:
            ...
            if hedge_qty != 0:
                ...
                # Cross to take so the hedge actually fills.
                if hedge_qty > 0 and vf_ask is not None:
                    orders.setdefault(vf_sym, []).append(Order(vf_sym, vf_ask, hedge_qty))
                elif hedge_qty < 0 and vf_bid is not None:
                    orders.setdefault(vf_sym, []).append(Order(vf_sym, vf_bid, hedge_qty))
