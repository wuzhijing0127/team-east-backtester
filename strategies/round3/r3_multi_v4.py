...
def trade_vev_near(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # v4 C1: CLUSTER PASSIVE UNWIND — rotation as first-priority when stuck.
    # When cluster_pos is extreme AND this strike is meaningfully loaded, post a
    # passive unwind at the best opposite-side price (joining the book, not crossing)
    # and RETURN. We skip take/maker logic for this strike on this tick.
    if in_cluster:
        if cluster_pos > 500 and pos > 80:
            unwind_qty = min(20, pos, limit + pos)
            if unwind_qty > 0 and best_ask is not None:
                orders.append(Order(product, best_ask, -unwind_qty))
                return orders
        elif cluster_pos < -500 and pos < -80:
            unwind_qty = min(20, -pos, limit - pos)
            if unwind_qty > 0 and best_bid is not None:
    ...
    # v4 C3: PER-STRIKE inventory cap for cluster (5000/5100/5200) —
    # catches single-strike inflation even when cluster_pos is below 450.
    if in_cluster:
        if pos > 240:
            buy_qty = 0
