"""
Round 3 Strategy — r3_multi_v4.4
================================
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # v4.4 H_v44_1: RESPONSIVE PASSIVE VF HEDGE (replaces v3.4 P3').
        # Lifts threshold-trigger from 500→400, weight 0.10→0.60, cap ±5→±20.
        # Still posts inside-market passively (never crosses) so the v3.3 spread-bleed
        # disaster cannot recur. Higher rate + bigger queue position = much faster
        # fill rate when basket delta swings through volatility.
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            if abs(total_delta) >= 400:
                ...
                target_vf = -round(0.60 * total_delta)
