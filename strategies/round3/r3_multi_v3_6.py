"""
Round 3 Strategy — r3_multi_v3.6
================================
v3.6 deltas vs v3.5 — three small HG dials to convert "doesn't catastrophically
hold but still bleeds" into "trades profitably and rotates inventory":

  J1. Lower one-sided threshold: 50 -> 30. Bot goes passive sell-only at +30
      (was +50) and passive buy-only at -30 (was -50). Inventory rotation
      kicks in earlier so positions don't have time to grow.
  J2. Boost the unwind-side maker quote size when over the threshold —
      sell_qty floored at 30 when long, buy_qty floored at 30 when short.
      Faster passive shed without crossing.
  J3. HG k_inv 37.5 -> 50. Stronger reservation lean against existing pos
      so the passive bid moves further away (when long) and ask comes closer
      to mid, raising fill probability on the unwind side.

VF and all VEV engines still IDENTICAL to v3.4.

v3.5 deltas vs v3.4 — HG-only, passive-first inventory rotation.
...
HYDROGEL_PARAMS = {
    ...
    "take_edge": 4,
    "half_spread": 2,
    "k_inv": 50.0,        # v3.6 J3: 37.5 -> 50, stronger reservation skew
...
def trade_hydrogel(state: TradingState, ts: dict) -> List[Order]:
    ...
    # v3.6 J2: BOOST the unwind side when over threshold so we shed faster.
    # Applies whether or not we're in trend mode.
    if pos > 30:
        sell_qty = min(max(sell_qty, 30), limit + pos)
    elif pos < -30:
        buy_qty = min(max(buy_qty, 30), limit - pos)

    # v3.6 J1: POSITION-AWARE H1 with threshold 30 (was 50 in v3.5).
