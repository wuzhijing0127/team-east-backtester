"""
Round 3 Strategy — r3_multi_v5
==============================
v5 deltas vs v3.4 — replaces the chronic-loss HG engine with the conservative
passive-MM approach that scored +607 on HG in r3_asset_only_v1.log:

  v5_HG. PASSIVE-ONLY HG MARKET MAKER (replaces v3.2/v3.4 HG engine).
         - Fair = mid + microprice_tilt + imbalance_signal × spread
           (no anchor, no drift bias, no trend regime, no risk_mult)
         - half_spread = 3 (wider quotes, less adverse selection)
         - base_size = 20
         - take_edge = 8 (only takes on large dislocations — was 4 in v3.4)
         - soft_cap = 150 (leaves 50-lot buffer to limit; was 200)
         - skew formula: k_inv * (pos/soft_cap) * (half_spread*2) with k_inv=0.10
           → at pos=150 the maker quotes shift only 0.6 ticks (was 10+ in v3.4)
         - Quotes inside the existing book by 1 tick when allowed.
         Asset_only_v1 evidence: 20 trades over the day, avg buy 9974.17 vs
         avg sell 9981.00 → +6.83 ticks/trade SPREAD CAPTURE (not bleed).

VF and all VEV engines are IDENTICAL to v3.4 (cluster controls P1/P2/P4,
passive VF hedge P3', deep-ITM/far-OTM, etc).

A separate v5 with explicit BS pricing was preserved at r3_multi_v5_bs.py.

v3.4 deltas vs v3.3 — v3.3 backtest: VEV cluster controls (P1/P2/P4) worked
well (+4.3k on the option cluster), but P3's aggressive crossing VF hedge
3x'd VF turnover and bled ~24k in spread cost. Net regressed -20k vs v3.2.
...
# ============================================================
# HYDROGEL_PACK — passive flow-driven MM (asset_only_v1 style, +607 PnL)
# ============================================================
...
def trade_hydrogel(state: TradingState, ts: dict) -> List[Order]:
    ...
    # Reservation skewed lightly by inventory (tiny: 0.6 ticks at full soft_cap).
    import math as _m
    skew = p["k_inv"] * (pos / max(1, soft_cap)) * max(2, half_spread * 2)
    res = fair - skew
    bid_price = int(_m.floor(res - half_spread))
    ask_price = int(_m.ceil(res + half_spread))

    # Place inside the book by 1 tick when it improves the quote
