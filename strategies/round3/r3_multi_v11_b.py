"""
Round 3 Strategy — r3_multi_v11_b (hedge tuning)
=================================================
Branch B from v10_blend: keep the trigger at |total_delta|≥150 (proven helpful)
but reduce hedge intensity since BS deltas can be off when our fair disagrees
with market.

  trigger:  150    (unchanged — directional from v9 evidence)
  weight:   0.60   (was 0.95) — don't fully neutralize an uncertain delta estimate
  cap:      ±20    (was ±30) — gentler per-tick adjustment
  posting:  PASSIVE inside-market (unchanged — never crosses)

Other v10_blend pieces preserved: market-anchored 0.3/0.7 blend, bisection
IV for delta, inventory shading.
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            if abs(total_delta) >= 150:  # v10_blend Q3_KEPT: 250 -> 150
                ...
                target_vf = -round(0.60 * total_delta)  # v11_b: 0.95 -> 0.60
                hedge_qty = max(-20, min(20, target_vf - vf_pos))  # v11_b: ±30 -> ±20
                if hedge_qty > 0:
