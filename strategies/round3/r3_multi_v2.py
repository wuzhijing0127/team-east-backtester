"""
Round 3 Strategy — r3_multi_v2
==============================
v2 deltas vs v1:
  1. Theta bias on near/OTM VEVs (asymmetric long/short adjustment).
  2. Cross-strike monotonicity guard on near VEV fair (C(K) >= C(K+100)).
  3. OTM base_size 15 -> 10 for safety.
  4. Edge filter on aggressive takes: +1 extra tick required (fade pure noise).
  5. Deep-ITM fair includes +2 time-value premium.
  6. HYDROGEL drift-following bias (0.3 * one-tick mid drift) + micro_beta 0.05->0.15.
  7. VELVETFRUIT_EXTRACT drift-following bias (0.2 * one-tick mid drift).
  8. Near-VEV large-long penalty: fair -= 1.5 when pos > 50.
...
VEV_NEAR_PARAMS = {
    ...
    "base_size": 10,      # v2: reduced 15->10 to limit OTM blow-up risk
    "tier_extreme": 0.85,
}
...
def trade_vev_near(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # --- v2 mod #1: theta bias (fade the EMA lag for OTM) ---
    # Stronger decay further OTM; asymmetric so we don't double-shift symmetric.
    theta_decay = 0.15 if strike >= 5400 else 0.08
    if pos >= 0:
        fair -= theta_decay
    else:
    ...
    # --- v2 mod #2: cross-strike monotonicity (C(K) >= C(K+100), step=100 in near tier) ---
    lower_key = f"vev{strike - 100}_fair"
    higher_key = f"vev{strike + 100}_fair"
    if higher_key in ts:
        fair = min(fair, ts[higher_key] + 50)
    if lower_key in ts:
        fair = max(fair, ts[lower_key] - 50)
