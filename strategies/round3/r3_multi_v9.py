"""
Round 3 Strategy — r3_multi_v9
==============================
v9 deltas vs v8 — three fixes targeting the VEV_5400 t=59,700 PnL bleed
caused by static-smile drift:

  Q1. MID-ANCHORED VOLATILITY OFFSET. Track a global `vol_offset` (shift
      of the smile constant term c) updated each tick from the deviation
      between market mid and BS fair on a liquid ATM strike (VEV_5300).
      Update rule: vol_offset += 0.005 * (atm_mid - bs_fair_atm) when the
      deviation exceeds 1.0 tick. Capped at ±0.02 to prevent runaway.
      Stored in `ts["vol_offset"]`.
  Q2. INVENTORY SKEW SHADING for options. When |pos| > 200 on any BS-traded
      strike, shade fair against position linearly: shade = (|pos|-200)/100,
      so 0 ticks at pos=200, 1 tick at pos=300. Discourages topping up near
      limit and accelerates passive unwind without crossing.
  Q3. TIGHTER DELTA-HEDGE TRIGGER. |total_delta| ≥ 150 (was 250). Hedge
      fires more often during vol spikes to bleed off underlying exposure
      before option PnL diverges. Weight (0.95) and cap (±30) unchanged.

PRESERVED FROM v8: SMILE_STRIKES = (4000..5500), trade_vev_bs (was
trade_vev_near), RESID_AGGRESSION = 0.75, far-OTM short-only, HG flow MM
with imb_coef = 0.55. HG fair stays purely flow-driven (no anchor).

Original v8 docstring follows for reference:
...
def smile_iv(K: float, S: float, T_days: float, vol_offset: float = 0.0) -> float:
    """Fitted parabolic smile in normalized log-moneyness, with optional vol_offset.
    The offset shifts the constant term c (vertical translation of the curve)."""
    if T_days <= 0 or S <= 0 or K <= 0:
        return SMILE_C + vol_offset
    m = math.log(K / S) / math.sqrt(T_days)
    sigma = SMILE_A * m * m + SMILE_B * m + (SMILE_C + vol_offset)
    return max(0.001, sigma)


# v9 Q1: ATM-anchored vol_offset bounds.
VOL_OFFSET_CAP = 0.02         # max ±shift on the smile constant term
VOL_OFFSET_LR = 0.005         # learning rate per tick when above the deadband
VOL_OFFSET_DEADBAND = 1.0     # ticks of mid-vs-BS deviation before adapting
ATM_REF_STRIKE = 5300         # liquid ATM-ish strike used as the anchor


def update_vol_offset(state, ts: dict, vf_mid):
    """Adjust ts['vol_offset'] by comparing market mid of ATM_REF_STRIKE against
    the smile-implied BS price. Adapts slowly toward whatever offset would close
    the gap; capped to avoid runaway. Mirrors the spec's 'mid-anchored offset'."""
    cur = ts.get("vol_offset", 0.0)
    if vf_mid is None:
        return cur
    od = state.order_depths.get(f"VEV_{ATM_REF_STRIKE}")
    if od is None:
        return cur
    bid = max(od.buy_orders) if od.buy_orders else None
    ask = min(od.sell_orders) if od.sell_orders else None
    ...
    sigma_atm = smile_iv(ATM_REF_STRIKE, vf_mid, T, cur)
    bs_fair_atm = bs_call_price(vf_mid, ATM_REF_STRIKE, T, sigma_atm)
    deviation = atm_mid - bs_fair_atm
    if abs(deviation) > VOL_OFFSET_DEADBAND:
        cur = cur + VOL_OFFSET_LR * deviation
        cur = max(-VOL_OFFSET_CAP, min(VOL_OFFSET_CAP, cur))
        ts["vol_offset"] = cur
    return cur


def get_total_vev_delta(state, ts: dict = None) -> float:
    """Sum pos*delta using strike-specific smile sigmas when context available."""
    total = 0.0
    if ts is not None:
        vf_mid = ts.get("vf_mid")
        if vf_mid is not None:
            T = get_tte_days(state.timestamp)
            voff = ts.get("vol_offset", 0.0)
            for strike in VEV_DELTA:
                pos = state.position.get(f"VEV_{strike}", 0)
                if pos == 0:
                    continue
                sigma_k = smile_iv(strike, vf_mid, T, voff)
                d = bs_call_delta(vf_mid, strike, T, sigma_k)
                total += pos * d
            return total
    # Fallback: static empirical deltas
...
def trade_vev_deep_itm(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    voff = ts.get("vol_offset", 0.0)
    sigma_k = smile_iv(strike, vf_mid, T_days, voff)
...
def trade_vev_bs(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # v6 PHASE 1+2 + v9 Q1: pure BS fair from fitted smile, with mid-anchored vol_offset.
    vf_mid = ts.get("vf_mid")
    T_days = get_tte_days(timestamp)
    ...
    # v9 Q2: INVENTORY SKEW SHADING. When |pos| approaches limit, shade fair
    # against the position by up to 1 tick to discourage topping up and
    # accelerate passive unwind.
    if abs(pos) > 200:
    ...
    # v6 PHASE 2: 1-sigma residual entry filter (computed below; used when
    # taking/quoting). Threshold in PRICE units = vega · SMILE_RESID_SD.
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # v9 Q1: refresh vol_offset BEFORE option engines consume it.
        update_vol_offset(state, ts, ts.get("vf_mid"))
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            if abs(total_delta) >= 150:  # v9 Q3: 250 -> 150, hedge sooner
