"""
Round 3 Strategy — r3_multi_v10 (safe rollback)
================================================
v10 = v8 base (which scored +2,975) + the two safe parts of v9:
  Q2_KEPT. Inventory shading: when |pos| > 200 on a BS-traded strike, shade
           fair against pos by linear shade = (|pos|-200)/100, capped at 1 tick.
  Q3_KEPT. Hedge trigger lowered from |total_delta|≥250 to ≥150. v9 evidence:
           VF PnL went from +2,255 to +2,758 with this change.

DROPPED FROM v9: the vol_offset adapter (Q1). It saturated at the +0.02 cap
within day 0 and pushed BS prices ~5–30 ticks above market, causing the bot
to over-buy options at the ask. Result was −264k vs v8.

The v8 docstring follows for the rest of the lineage:
...
def trade_vev_bs(state: TradingState, ts: dict, strike: int) -> List[Order]:
    ...
    # v10 Q2_KEPT: inventory shading when approaching limit
    if abs(pos) > 200:
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if vf_sym in state.order_depths and state.timestamp < UNWIND_START:
            ...
            if abs(total_delta) >= 150:  # v10 Q3_KEPT: 250 -> 150
