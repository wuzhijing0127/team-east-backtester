"""Code generator for the 182338 strategy template.

Base: PEPPER limit=80, pure buy-and-hold (never sell). Straight-line PnL = 7,286.
Variable: ASH market-making — toggled on with parameterized values to see if it adds PnL.
"""
...
DEFAULTS_182338: dict[str, Any] = {
    # ASH (enabled by default for grid search)
    "ash_enabled": True,
    ...
    # PEPPER (locked — do not grid search these)
    "pepper_limit": 80,
}

SEARCH_PARAMS_182338: dict[str, dict[str, Any]] = {
...
def merge_params_182338(overrides: dict[str, Any]) -> dict[str, Any]:
    params = dict(DEFAULTS_182338)
    for k, v in overrides.items():
        if k not in DEFAULTS_182338:
            raise ValueError(f"Unknown 182338 parameter: {k}")
        params[k] = v
    return params


def params_to_name_182338(params: dict[str, Any]) -> str:
    diffs = []
    for k, v in sorted(params.items()):
        if k in DEFAULTS_182338 and v != DEFAULTS_182338[k]:
            short = k.replace("ash_", "a_").replace("pepper_", "p_")
            diffs.append(f"{short}_{v}")
    if not diffs:
        return "s182338_baseline"
    return "s182338_" + "_".join(diffs[:5])


def generate_strategy_182338(
    params: dict[str, Any],
    name: str | None = None,
    output_dir: str | Path = "generated",
) -> Path:
    if name is None:
        name = params_to_name_182338(params)
    code = _build_code_182338(params)
...
def _compact_diff(p: dict[str, Any]) -> str:
    ...
    for k, v in sorted(p.items()):
        if k in DEFAULTS_182338 and v != DEFAULTS_182338[k]:
            diffs.append(f"{k}={v}")
    return ", ".join(diffs) if diffs else "defaults (182338)"


def _build_code_182338(p: dict[str, Any]) -> str:
    ash_enabled = p.get("ash_enabled", True)

    # ASH trade call: either full MM or empty orders
    if ash_enabled:
        ash_dispatch = 'orders[product] = trade_ash(state, ts)'
    else:
        ash_dispatch = 'orders[product] = []'
