"""Strategy code generator — produces self-contained .py files from parameter dicts.

Uses v4o_L1_14.py as the template. The strategy logic (functions, Trader class) stays
identical; only the ASH_PARAMS and PEPPER_PARAMS dicts are replaced with injected values.
Each generated file is fully self-contained with no external imports beyond datamodel.
"""
...
# ── Default parameters (v4o_L1_14 baseline) ──────────────────────
...
DEFAULTS: dict[str, Any] = {
    ...
    "ash_micro_beta": 0.0,
    "ash_take_edge_buy": 2,
    "ash_take_edge_sell": 3,
    "ash_k_inv": 2.5,
    "ash_flatten_size": 10,
    "ash_tier_medium": 0.4,
    "ash_tier_high": 0.7,
    "ash_tier_extreme": 1.0,
    "ash_wide_spread_thr": 8,
    "ash_narrow_spread_thr": 4,
    "ash_L1_size": 14,
    "ash_L1_base_spread": 3,
    "ash_L2_spread": 4,
    "ash_L2_size": 0,
    "ash_L3_spread": 8,
    "ash_L3_size": 0,
    "ash_inv_bias_thr": 0.3,
    # PEPPER
    "pepper_position_limit": 50,
    "pepper_fair_slope": 0.001,
    "pepper_day_base_neg2": 9998,
    "pepper_day_base_neg1": 10998,
    "pepper_day_base_0": 11998,
    "pepper_buy_edge": 0,
    "pepper_dip_buy_edge": 3,
    "pepper_dip_threshold": 3,
    "pepper_take_profit_edge": 4,
    "pepper_min_long_frac": 0.96,
    "pepper_bid_spread": 4,
    "pepper_ask_spread": 8,
    "pepper_base_size": 10,
    "pepper_fair_sanity_max_dev": 20,
    "pepper_inv_tier_medium": 0.4,
    "pepper_inv_tier_high": 0.6,
    "pepper_inv_tier_extreme": 0.85,
    "pepper_wide_spread_thr": 8,
    "pepper_narrow_spread_thr": 25,
    "pepper_overlay_sell_thr": 0.5,
}

# Parameters worth searching over (subset of DEFAULTS)
SEARCH_PARAMS: dict[str, dict[str, Any]] = {
    "ash_take_edge_buy":      {"default": 2,    "type": "int",   "range": [1, 2, 3, 4]},
    "ash_take_edge_sell":     {"default": 3,    "type": "int",   "range": [2, 3, 4, 5]},
    "ash_k_inv":              {"default": 2.5,  "type": "float", "range": [1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]},
    "ash_L1_size":            {"default": 14,   "type": "int",   "range": [8, 10, 12, 14, 16, 18, 20, 25]},
    "ash_L1_base_spread":     {"default": 3,    "type": "int",   "range": [2, 3, 4, 5]},
    "ash_L2_size":            {"default": 0,    "type": "int",   "range": [0, 2, 5, 8]},
    "ash_L2_spread":          {"default": 4,    "type": "int",   "range": [3, 4, 5, 6]},
    "ash_wide_spread_thr":    {"default": 8,    "type": "int",   "range": [6, 8, 10]},
    "ash_narrow_spread_thr":  {"default": 4,    "type": "int",   "range": [3, 4, 5, 6]},
    "ash_tier_medium":        {"default": 0.4,  "type": "float", "range": [0.3, 0.4, 0.5]},
    "ash_tier_high":          {"default": 0.7,  "type": "float", "range": [0.6, 0.7, 0.8]},
    "ash_inv_bias_thr":       {"default": 0.3,  "type": "float", "range": [0.2, 0.3, 0.4, 0.5]},
    "pepper_dip_buy_edge":    {"default": 3,    "type": "int",   "range": [1, 2, 3, 4, 5]},
    "pepper_dip_threshold":   {"default": 3,    "type": "int",   "range": [2, 3, 4, 5]},
    "pepper_take_profit_edge":{"default": 4,    "type": "int",   "range": [3, 4, 5, 6, 8]},
    "pepper_min_long_frac":   {"default": 0.96, "type": "float", "range": [0.80, 0.88, 0.92, 0.96, 1.0]},
    "pepper_bid_spread":      {"default": 4,    "type": "int",   "range": [2, 3, 4, 5, 6]},
}


def merge_params(overrides: dict[str, Any]) -> dict[str, Any]:
    """Merge overrides into defaults. Only keys present in DEFAULTS are accepted."""
...
def params_to_name(params: dict[str, Any]) -> str:
    """Generate a short descriptive name from parameters that differ from defaults."""
    diffs = []
    ...
    if not diffs:
        return "v4o_baseline"
    return "_".join(diffs[:5])  # cap length
...
def generate_strategy(
    ...
    """Generate a complete self-contained strategy .py file.

    Args:
        params: Full parameter dict (use merge_params() to fill defaults).
        name: Filename stem. Auto-generated from params if None.
...
def _build_strategy_code(p: dict[str, Any]) -> str:
    """Build the full strategy source code with injected parameters."""
    return f'''\
# Auto-generated strategy — optimizer variant
# Params: {_compact_diff(p)}
...
ASH_PARAMS = {{
    ...
    "take_edge_buy": {p["ash_take_edge_buy"]},
    "take_edge_sell": {p["ash_take_edge_sell"]},
    "k_inv": {p["ash_k_inv"]},
    "flatten_size": {p["ash_flatten_size"]},
    "tier_medium": {p["ash_tier_medium"]},
    "tier_high": {p["ash_tier_high"]},
    "tier_extreme": {p["ash_tier_extreme"]},
    "wide_spread_thr": {p["ash_wide_spread_thr"]},
    "narrow_spread_thr": {p["ash_narrow_spread_thr"]},
    "L1_size": {p["ash_L1_size"]},
    "L1_base_spread": {p["ash_L1_base_spread"]},
    "L2_spread": {p["ash_L2_spread"]},
    "L2_size": {p["ash_L2_size"]},
    "L3_spread": {p["ash_L3_spread"]},
    "L3_size": {p["ash_L3_size"]},
    "inv_bias_thr": {p["ash_inv_bias_thr"]},
}}

PEPPER_PARAMS = {{
    "position_limit": {p["pepper_position_limit"]},
    "fair_slope": {p["pepper_fair_slope"]},
    "day_base_map": {{-2: {p["pepper_day_base_neg2"]}, -1: {p["pepper_day_base_neg1"]}, 0: {p["pepper_day_base_0"]}}},
    "buy_edge": {p["pepper_buy_edge"]},
    "dip_buy_edge": {p["pepper_dip_buy_edge"]},
    "dip_threshold": {p["pepper_dip_threshold"]},
    "take_profit_edge": {p["pepper_take_profit_edge"]},
    "min_long_frac": {p["pepper_min_long_frac"]},
    "bid_spread": {p["pepper_bid_spread"]},
    "ask_spread": {p["pepper_ask_spread"]},
    "base_size": {p["pepper_base_size"]},
    "fair_sanity_max_dev": {p["pepper_fair_sanity_max_dev"]},
    "inventory_tiers": {{
        "medium": {p["pepper_inv_tier_medium"]},
        "high": {p["pepper_inv_tier_high"]},
        "extreme": {p["pepper_inv_tier_extreme"]},
    }},
    "wide_spread_thr": {p["pepper_wide_spread_thr"]},
    "narrow_spread_thr": {p["pepper_narrow_spread_thr"]},
    "overlay_sell_thr": {p["pepper_overlay_sell_thr"]},
}}
...
def _compact_diff(p: dict[str, Any]) -> str:
    """Short string showing params that differ from defaults."""
    diffs = []
    for k, v in sorted(p.items()):
        if k in DEFAULTS and v != DEFAULTS[k]:
            diffs.append(f"{k}={v}")
    return ", ".join(diffs) if diffs else "defaults (v4o_L1_14)"
