...
# ── PARAMS_BLOCK_START ──
VARIANT_ID = 'p5a_kinv1_stoploss_unwind'
VARIANT_LABEL = 'HG k_inv=1.0 + stop(pos=60,dd=1500,t=20,sz=30) + 3-layer unwind'
...
HG_PARAMS = {
    ...
    'stop_pos_threshold': 60,
    'stop_drawdown_threshold': 1500.0,
    'stop_min_ticks_at_pos': 20,
    'stop_unwind_size': 30,
}
...
VF_PARAMS = {
    ...
    'vel_alpha': 0.1,
    'revert_gate_threshold': 0.0,
    'stop_pos_threshold': 0,
    'stop_drawdown_threshold': 800.0,
...
def trade_flow_mm(state: TradingState, ts: dict, product: str, p: dict) -> List[Order]:
    ...
    # ── 2-layer end-of-session unwind (env-agnostic) ──
    # Layer 1: auto-detect per-day period from a timestamp rollover (current<prev).
    # Once detected, fire in last 1% of every detected period.
    # Layer 2: day-1 fallback before period is detected.
    #   • Fire in last 1k of [0, 100k) — covers our 100k env day 1.
    #   • Fire in last 2k of [0, 1M) — covers 1M env day 1.
    #   • Don't fire in 100k <= ts < 998k while period unknown (ambiguous).
    g_prev = ts.get("_g_prev_ts", -1)
    g_peak = ts.get("_g_day_peak_ts", 0)
    detected_period = ts.get("_g_detected_period", None)

    if g_prev >= 0 and state.timestamp < g_prev:
        # timestamp rolled over -> per-day reset env detected
        if detected_period is None:
            # period ≈ previous-day peak + step
            detected_period = g_peak + 100
            ts["_g_detected_period"] = detected_period
        g_peak = state.timestamp

    g_peak = max(g_peak, state.timestamp)
    ts["_g_prev_ts"] = state.timestamp
    ts["_g_day_peak_ts"] = g_peak

    layer1_fire = False
    if detected_period is not None and detected_period > 0:
        in_day = state.timestamp % detected_period
        layer1_fire = in_day >= detected_period - max(1000, detected_period // 100)

    layer2_fire = False
    if detected_period is None:
        if state.timestamp < 100_000 and state.timestamp >= 99_000:
            layer2_fire = True  # 100k env, day 1 only
        elif state.timestamp >= 998_000:
            layer2_fire = True  # 1M env or accumulating envs at 1M chunks

    if layer1_fire or layer2_fire:
...
# nonce: 2026-04-25T23:48:25.395252 844387180
