...
# ── PARAMS_BLOCK_START ──
VARIANT_ID = 'clean_alpha'
VARIANT_LABEL = 'Step 1: clean alpha stack — micro_trend removed; sizing unchanged from round_quote'
...
def trade_flow_mm(state: TradingState, ts: dict, product: str, p: dict) -> List[Order]:
    ...
    # ── Composite momentum signal (used for fair, target_pos, sizing) ──
    ...
    # ── Target position from momentum signal ──
    target_pos = max(-soft_cap, min(soft_cap,
                                      p["target_pos_scaler"] * momentum_signal))
    pos_gap = target_pos - pos

    # ── Aggressive takes (size scales with alpha conviction) ──
    take_size = p["alpha_take_size"] if is_strong else base_size
    for ap in sorted(od.sell_orders.keys()):
        if fair - ap < take_edge:
            break
        room = max(0, limit - pos)
        qty = min(abs(od.sell_orders[ap]), room, take_size)
        if qty > 0:
    ...
    for bp in sorted(od.buy_orders.keys(), reverse=True):
        if bp - fair < take_edge:
            break
        room = max(0, limit + pos)
        qty = min(od.buy_orders[bp], room, take_size)
        if qty > 0:
    ...
    # ── Asymmetric maker sizing — bigger toward target, smaller opposite ──
    room_buy = max(0, soft_cap - pos)
    room_sell = max(0, soft_cap + pos)
    if abs(pos_gap) >= p["pos_gap_threshold"]:
        if pos_gap > 0:  # need to buy (toward target)
            b_size_target = p["alpha_quote_size"]
            a_size_target = p["mm_size_reduced"]
        else:            # need to sell
            b_size_target = p["mm_size_reduced"]
            a_size_target = p["alpha_quote_size"]
    else:
        b_size_target = a_size_target = base_size

    b_size = min(b_size_target, room_buy)
    a_size = min(a_size_target, room_sell)
...
# nonce: 2026-04-26T17:51:05.228673 248454747
